...
const openPage = (project, name) => {
  const projectPath = bamjuConfig.projects[project];
  const fn = normalizeName(name);
  const abs = path.join(projectPath, fn);
  const buf = fs.readFileSync(abs);
  const ret = buf.toString('UTF-8');

  return ret;
};

const normalizeName = name => `${name}.md`;